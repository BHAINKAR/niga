...
def check_account_stock(call):
    ...
    bot.send_message(call.message.chat.id, f"📦 Current Account Stock: {available_count} accounts available.")

def add_accounts(call):
    bot.send_message(call.message.chat.id, "Please send me the accounts to add, one per line.")
...
def process_add_accounts(message):
    ...
    bot.send_message(message.chat.id, f"✅ Added {len(formatted_accounts)} accounts.")

def remove_accounts(call):
    bot.send_message(call.message.chat.id, "Please send me the accounts to remove, one per line.")
...
def process_remove_accounts(message):
    ...
    bot.send_message(message.chat.id, f"✅ Removed {removed_count} accounts.")
...
def generate_account_as_user(call):
    ...
    if user_data[user_id]['last_generated'] and (now < user_data[user_id]['last_generated'] + timedelta(seconds=USER_COOLDOWN)):
        ...
        bot.answer_callback_query(call.id, f"⏳ Please wait {remaining_time.seconds} seconds.")
        return
    if not account_stock:
        bot.send_message(call.message.chat.id, "⚠️ No accounts to generate.")
        return
